#!/usr/bin/env python3
"""
Anki .apkg file parser - Generic parser for any Anki deck
...
This is a generic parser that works with any Anki .apkg file structure by:
1. Parsing card templates to determine which fields appear on front/back
2. Extracting all media references from all fields using comprehensive patterns
3. Supporting multiple media reference formats:
   - [sound:filename] syntax
   - <audio src="..."> HTML tags
   - <img src="..."> HTML tags  
   - Numeric media IDs (e.g., <img src="123">) mapped via media dictionary
   - Data attributes (data-src, data-media, etc.)
...
Output:
    - <apkg_filename>/: Directory named after the .apkg file (without extension)
        - cards.json: JSON file with all cards, each containing:
            - card_ids: List of all card IDs from the same note
...
Note: The parser extracts all media references that are explicitly present in the
fields. If Anki displays additional media through dynamic rendering or template
logic that isn't stored in the field data, those won't be captured.
"""
...
def generate_json_from_database(db_path, output_dir, media_dict, extracted_media, images_dir, audio_dir):
    ...
    This is a generic parser that works with any Anki .apkg file structure.
    It extracts all media references (images and audio) from all fields that appear
    on the front and back of cards based on the card templates.
    
    Media references are found using multiple methods:
    1. [sound:filename] syntax - explicit audio file references
    2. <audio src="..."> tags - HTML audio tags
    3. <img src="..."> tags - HTML image tags
    4. Numeric media IDs - Anki can reference media by numeric ID (e.g., <img src="123">)
       where the ID maps to a filename in the media dictionary
    
    Args:
        db_path: Path to SQLite database file
        output_dir: Directory to save JSON file
        media_dict: Dictionary mapping media IDs (as strings) to filenames
        extracted_media: Dictionary mapping filenames to relative paths
    ...
    try:
        ...
        for note in notes:
            ...
            # Get all front and back field indices from ALL templates for this note type
            # This ensures cards from the same note have consistent front/back content
            all_front_field_indices = set()
            all_back_field_indices = set()
            
            if mid in template_info:
                # Union all fields from all templates for this note type
                for template_ord, template_data in template_info[mid].items():
                    all_front_field_indices.update(template_data['front_fields'])
                    all_back_field_indices.update(template_data['back_fields'])
            else:
                # Fallback: if no template info, use first field for front, rest for back
                if len(fields) > 0:
                    all_front_field_indices = {0}
                if len(fields) > 1:
                    all_back_field_indices = set(range(1, len(fields)))
            
            # Combine all fields that appear on front (across all templates)
            front_content_list = []
            all_front_content = ''
            for field_idx in sorted(all_front_field_indices):  # Sort for consistent order
                if field_idx < len(fields) and fields[field_idx]:
                    field_content = fields[field_idx]
                    front_content_list.append(field_content)
                    all_front_content += ' ' + field_content if all_front_content else field_content
            
            # Combine all fields that appear on back (across all templates)
            back_content_list = []
            all_back_content = ''
            for field_idx in sorted(all_back_field_indices):  # Sort for consistent order
                if field_idx < len(fields) and fields[field_idx]:
                    field_content = fields[field_idx]
                    back_content_list.append(field_content)
                    all_back_content += ' ' + field_content if all_back_content else field_content
            
            # Extract images and audio from front and back content
            # Extract from combined content for comprehensive detection
            ...
            # Extract audio from each field individually to ensure we catch all references
            # This is important because audio might be referenced in multiple ways across different fields
            for field_idx in sorted(all_back_field_indices):
                if field_idx < len(fields) and fields[field_idx]:
                    field_content = fields[field_idx]
                    field_audio = extract_audio_from_content(field_content, media_dict, extracted_media)
                    # Add any new audio files found in this field (avoid duplicates)
                    for audio in field_audio:
                        # Normalize paths for comparison
                        audio_normalized = audio.replace('\\', '/').lower()
                        back_audio_normalized = [a.replace('\\', '/').lower() for a in back_audio]
                        if audio_normalized not in back_audio_normalized:
                            back_audio.append(audio)
            
            # Extract audio from each front field individually
            for field_idx in sorted(all_front_field_indices):
                if field_idx < len(fields) and fields[field_idx]:
                    field_content = fields[field_idx]
                    field_audio = extract_audio_from_content(field_content, media_dict, extracted_media)
                    # Add any new audio files found in this field (avoid duplicates)
                    for audio in field_audio:
                        # Normalize paths for comparison
                        audio_normalized = audio.replace('\\', '/').lower()
                        front_audio_normalized = [a.replace('\\', '/').lower() for a in front_audio]
                        if audio_normalized not in front_audio_normalized:
                            front_audio.append(audio)
            
            # Create ONE entry per note (not per card)
            # Use the first card ID as the key, but include all card IDs in the data
            if nid in note_to_cards and note_to_cards[nid]:
                # Use the first card's ID as the primary key
                first_card = note_to_cards[nid][0]
                card_id = str(first_card['id'])
                
                # Collect all card IDs from this note
                all_card_ids = [str(card['id']) for card in note_to_cards[nid]]
                
                # Create single entry for this note with all card IDs
                cards_data[card_id] = {
                    'card_ids': all_card_ids,  # List of all card IDs from this note
                    'front': {
                        'content': front_content_list,  # List of field contents
                        'media': {
                            'images': front_images,
                            'audio': front_audio
                        }
                    ...
                    'back': {
                        'content': back_content_list,  # List of field contents
                        'media': {
                            'images': back_images,
                            'audio': back_audio
                        }
...
def extract_audio_from_content(content, media_dict=None, extracted_media=None):
    """
    Extract ALL audio references from content using comprehensive parsing.
    This function checks for all possible ways Anki can reference audio files.
    ...
    Returns:
        List of audio file paths (relative to output directory)
    """
    audio_files = []
    audio_extensions = ['.mp3', '.ogg', '.wav', '.m4a', '.flac', '.aac', '.opus']
    ...
    # Method 2: Find Anki [sound:filename] references
    # Pattern: [sound:filename.mp3] - can appear multiple times in same field
    ...
    # Method 3: Find numeric media IDs that reference audio files
    # Anki can reference media by numeric ID: <img src="123"> or <audio src="123">
    # The media_dict maps these IDs to filenames
    if media_dict:
        # Check for numeric IDs in ANY HTML tag's src attribute
        # This catches: <img src="123">, <audio src="123">, <source src="123">, etc.
        numeric_src_pattern = r'<[^>]+src=["\'](\d+)["\']'
        numeric_matches = re.findall(numeric_src_pattern, content, re.IGNORECASE)
        for media_id in numeric_matches:
            if media_id in media_dict:
                filename = media_dict[media_id]
        ...
        # Method 4: Check for numeric IDs in data attributes or other HTML attributes
        # Pattern: data-src="123", data-media="123", etc.
        ...
        for pattern in data_attribute_patterns:
    ...
    for audio in audio_files:
        # Normalize path for comparison (handle both forward and backward slashes)
        audio_normalized = audio.replace('\\', '/')
        if audio_normalized not in seen:
            seen.add(audio_normalized)
