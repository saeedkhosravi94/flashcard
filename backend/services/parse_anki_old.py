...
def generate_json_from_database(db_path, output_dir, media_dict, extracted_media, images_dir, audio_dir):
    ...
    try:
        ...
        # Get model information for field names
        model_info = {}
        ...
        for note in notes:
            ...
            if mid in model_info:
                model_fields = model_info[mid]['fields']
            ...
            # Collect all content for front and back (including additional fields)
            # This handles cases where audio might be in separate fields
            all_front_content = front_content
            all_back_content = back_content
            
            # Check all fields for audio references and add them to appropriate side
            # Fields that contain 'audio' in their name (case-insensitive) are likely audio fields
            if mid in model_info:
                for i, field_name in enumerate(model_info[mid]['fields']):
                    if i < len(fields) and fields[i]:
                        field_content = fields[i]
                        # If field name suggests it's front audio or German audio, add to front
                        field_lower = field_name.lower()
                        if 'audio' in field_lower:
                            if 'english' in field_lower or 'back' in field_lower:
                                # English/back audio goes to back
                                all_back_content += ' ' + field_content
                            else:
                                # Other audio (German/front) goes to front
                                all_front_content += ' ' + field_content
                        # Also check if field content itself contains audio references
                        elif '[sound:' in field_content or '<audio' in field_content:
                            # If it's an early field (0-2), assume front; otherwise back
                            if i <= 1:
                                all_front_content += ' ' + field_content
                            else:
                                all_back_content += ' ' + field_content
            ...
            if nid in note_to_cards:
                for card in note_to_cards[nid]:
                    ...
                    # Extract images and audio from front (including all front-related content)
                    ...
                    # Extract images and audio from back (including all back-related content)
                    ...
                    cards_data[card_id] = {
                        'front': {
                            'content': front_content,  # Store original front content without audio field content
                            'media': {
                                'images': front_images,
                                'audio': front_audio
                            }
                        ...
                        'back': {
                            'content': back_content,  # Store original back content without audio field content
                            'media': {
                                'images': back_images,
                                'audio': back_audio
                            }
