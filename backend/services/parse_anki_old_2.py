...
def generate_json_from_database(db_path, output_dir, media_dict, extracted_media, images_dir, audio_dir):
    ...
    try:
        ...
        for note in notes:
            ...
            # Extract images and audio from front and back content separately
            ...
            # Create cards for this note - all cards from the same note will have the same content
            if nid in note_to_cards:
                for card in note_to_cards[nid]:
                    card_id = str(card['id'])
                    
                    # All cards from the same note get the same content
                    cards_data[card_id] = {
                        'front': {
